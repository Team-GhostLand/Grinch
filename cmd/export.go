...
func init() {
	...
	exportCmd.Flags().Bool("default", false, "Default mode: Makes your modpack a fully standards-compliant Mrpack, that works flawlessly in both standards-compliant client launchers (that's important for later) and server-side Modrinth loaders. Replaces the non-standard (but much easier for us to operate on) server_overrides/REMOVALS.txt file with a proper client_overrides/ directory. Anything else (like what mods are optional, etc.) should alreadly be configured to be standards-compliant, if you used grinch import on an earlier-grinch-exported project (*if not, please refer to the docs to find out how to „Grinchify” an Mrpack). You don't need to set this flag explicitly - in fact, the code doesn't even check for its presence (apart from making sure it wasn't combined with other mode flags), due to the fact that this is the default behaviour. Cannot be combined with other mode flags.")
