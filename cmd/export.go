...
var exportCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		//--PARSING ARGS/FLAGS--
		mode_flag_parse_error := "Couldn't determine export mode"
...
func init() {
	...
	exportCmd.Flags().Bool("default", false, "Default mode: Makes your modpack a fully standards-compliant Mrpack, that works flawlessly in both standards-compliant client launchers (that's important for later) and server-side Modrinth loaders. Replaces the non-standard (but much easier for us to operate on) server-overrides/REMOVALS.txt file with a proper client-overrides/ directory. Anything else (like what mods are optional, etc.) should already be configured to be standards-compliant, if you used grinch import correctly (please do grinch import --help for more details). You don't need to set this flag explicitly - in fact, the code doesn't even check for its presence (apart from making sure it wasn't combined with other mode flags), due to the fact that this is the default behavior. Cannot be combined with other mode flags.")
	...
	exportCmd.Flags().BoolP("dev", "d", false, "Dev mode: Transforms JSON and files in such a way that makes your pack suitable for development using Modrinth's official launcher. Replaces server-overrides/ with overrides/.SERVERSIDE (because otherwise Modrinth launcher would discard them, due to it being a client-side launcher) and marks any client-unsupported mod as client-required (same reason). These steps will later be reversed when running grinch import (though the latter requires you to have prefixes configured correctly - please do grinch import --help for more details). Cannot be combined with other mode flags.")
	exportCmd.Flags().BoolP("slim", "s", false, "Slim mode: Marks every client-optional mod as client-unsupported. This is done to mimic the standards-compliant behavior of „letting users choose whether they want to install optional mods” within the Modrinth launcher (which - hilariously - isn't compliant with Modrinth's own standard), by letting the users pick whether they want a „full” modpack experience or a „slim” one (if you provide both files, of course). Since this option only targets clients (or, one specific client, really), we don't do any extra transforms to ensure proper server support (ie. server-overrides/REMOVALS.txt stays). Cannot be combined with other mode flags.")
	exportCmd.Flags().BoolP("tweakable", "t", false, "Tweakable mode: Marks every client-optional mod as disabled. This is done to mimic the standards-compliant behavior of „letting users choose whether they want to install optional mods” within the Modrinth launcher (which - hilariously - isn't compliant with Modrinth's own standard), by letting the users manually re-enable mods that they want. Since this option only targets clients (for that matter, THIS MODE IS COMPLETELY INCOMPATIBLE WITH SERVERS!!!, as mod disabling targets both sides, so server-required mods may get hit by accident), we don't do any extra transforms to ensure proper server support (ie. server-overrides/REMOVALS.txt stays). Cannot be combined with other mode flags.")
